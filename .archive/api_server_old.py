...
def run_complete_pipeline(requirement: str, skip_clarification: bool = False) -> Dict[str, Any]:
    """
    Run the complete CrewBuilder pipeline using proper CrewAI patterns
    
    Pipeline: Clarification → Architecture → Code → Review → Deployment → Documentation
    """
    
    try:
        # Use the new CrewBuilder V2
        from crewbuilder_crew_v2 import get_crewbuilder_v2
        crewbuilder = get_crewbuilder_v2()
        
        # Run the complete system building
        print("🚀 Running CrewBuilder V2...")
        results = crewbuilder.build_from_requirement(requirement)
        
        # Format results for frontend compatibility
        if results['success']:
            pipeline_results = format_crewbuilder_v2_results(results)
        ...
        # No fallback - if CrewBuilder V2 fails, we should fix it, not use bad patterns
        raise HTTPException(
            status_code=500,
            detail=f"CrewBuilder V2 failed: {str(e)}. Please check logs."
        )

def format_crewbuilder_v2_results(results: Dict[str, Any]) -> Dict[str, Any]:
    """Format CrewBuilder V2 results for frontend compatibility"""
    
    # Extract system name from requirement
    system_name = extract_system_name(results.get('requirement', 'System'))
    
    # Parse architecture for agent count
    architecture = results.get('architecture', '')
    agent_count = architecture.lower().count('agent:')
    if agent_count == 0:
        agent_count = 5  # Default
    
    # Determine complexity from review
    review = results.get('quality_review', '')
    complexity = 'moderate'
    if 'simple' in review.lower() or agent_count <= 3:
        complexity = 'simple'
    elif 'complex' in review.lower() or agent_count >= 7:
        complexity = 'complex'
    ...
    return {
        ...
        'estimatedTime': f"{results.get('execution_time', 0):.1f} seconds",
        'estimatedCost': '$50-100',  # TODO: Calculate from API report
        'architecture': {
            'summary': results.get('architecture', '')[:500],
            'full': results.get('architecture', '')
        },
        'deployment': {
            'config': results.get('deployment_config', ''),
            'instructions': 'See deployment configuration above'
        },
        'generatedAt': datetime.now().isoformat(),
        'complexity_score': get_complexity_score(complexity),
        'pipeline_stages': [
            {
                'stage': 'Clarification',
                'status': 'completed',
                'output': results.get('clarification_questions', '')
            },
            {
                'stage': 'API Verification',
                'status': 'completed',
                'output': results.get('api_report', '')
            },
            {
                'stage': 'Architecture Design',
                'status': 'completed',
                'output': results.get('architecture', '')
            },
            {
                'stage': 'Code Generation',
                'status': 'completed',
                'output': f"Generated {len(results.get('generated_code', ''))} characters"
            },
            {
                'stage': 'Quality Review',
                'status': 'completed',
                'output': results.get('quality_review', '')
            },
            {
                'stage': 'Documentation',
                'status': 'completed',
        ...
        'requirements_txt': results.get('requirements_txt', ''),
        'documentation': results.get('documentation', ''),
        'success': True
    }

def format_orchestrator_results(results: Dict[str, Any]) -> Dict[str, Any]:
    """Format orchestrator results for frontend compatibility"""
    
    # Extract key information from task outputs
    task_outputs = results.get('task_outputs', {})
    
    # Parse requirements analysis
    req_output = task_outputs.get('requirements_analysis', {}).get('output', '')
    complexity = 'moderate'
    estimated_agents = 5
    
    # Try to extract complexity and agent count
    if 'simple' in req_output.lower():
        complexity = 'simple'
        estimated_agents = 3
    elif 'complex' in req_output.lower():
        complexity = 'complex'
        estimated_agents = 8
    
    # Parse architecture
    arch_output = task_outputs.get('system_architecture', {}).get('output', '')
    
    # Parse generated code
    code_output = task_outputs.get('code_generation', {}).get('output', '')
    
    return {
        'systemName': extract_system_name(results.get('user_requirement', 'System')),
        'agents': estimated_agents,
        'complexity': complexity,
        'estimatedTime': f"{results.get('execution_time', 0):.1f} seconds",
        'estimatedCost': '$50-100',  # TODO: Calculate from API usage
        'architecture': {
            'summary': arch_output[:500] if arch_output else 'Architecture designed',
            'crew_name': 'AI Agent Crew',
            'workflow': 'hierarchical'
        },
        'deployment': {
            'instructions': task_outputs.get('deployment_config', {}).get('output', ''),
            'hosting': task_outputs.get('hosting_setup', {}).get('output', ''),
            'monitoring': task_outputs.get('monitoring_setup', {}).get('output', '')
        },
        'generatedAt': results.get('generated_at'),
        'complexity_score': get_complexity_score(complexity),
        'pipeline_stages': [
            {
        ...
        'requirements_txt': results.get('requirements_txt', 'crewai>=0.1.0\nlangchain>=0.1.0'),
        'documentation': task_outputs.get('documentation', {}).get('output', ''),
        'success': results.get('success', True)
    }

def run_sequential_pipeline_fallback(requirement: str) -> Dict[str, Any]:
    """Fallback to sequential pipeline if orchestrator fails"""
...
async def clarify_chat(request: dict):
    """
    Conversational clarification using CrewBuilder V2's clarification specialist
    """
    try:
        ...
        # Use CrewBuilder V2's clarification
        from crewbuilder_crew_v2 import get_crewbuilder_v2
        ...
        # Simple crew for single task
        crew = Crew(
            agents=[clarification_agent], 
            tasks=[task],
            verbose=False  # Less verbose for chat
        )
